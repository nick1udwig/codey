...
func (m *Manager) action(w http.ResponseWriter, r *http.Request, sid string, s session) error {
	...
	case "connect":
		...
		if provider == "nextcloudnotes" || provider == "caldav" {
	...
	case "bind", "apply":
		...
		m.page(w, s, "Step 4 of 4 · Active. Setup is complete. Open your collection on the watch and refresh after the first sync. You can close this setup page.", nil)
	case "recover":
