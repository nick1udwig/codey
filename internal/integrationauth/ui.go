...
var pageTemplate = template.Must(template.New("settings").Parse(`<!doctype html><meta name="viewport" content="width=device-width"><title>Notes and to-dos sync</title><style>body{font:16px system-ui;max-width:48rem;margin:2rem auto;padding:1rem}label{display:block;margin:1rem 0}input,select,button{font:inherit;padding:.5rem;max-width:100%}fieldset{margin:1rem 0}pre{white-space:pre-wrap;overflow-wrap:anywhere}</style><h1>Notes and to-dos sync</h1><p>{{.Message}}</p><p>Records are saved on this server. External services are optional. Disconnecting preserves server and remote records.</p>
...
func (m *Manager) page(w http.ResponseWriter, s session, message string, extra map[string]any) {
	...
	data := map[string]any{"Base": strings.TrimRight(m.Config.PublicURL, "/") + "/integrations", "CSRF": s.CSRF, "Message": message, "Collections": cols, "Providers": m.Descriptors(), "Status": string(p.Raw(status)), "Problems": problems, "Conflicts": conf}
	for k, v := range extra {
		data[k] = v
	}
