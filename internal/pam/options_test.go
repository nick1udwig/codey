...
func TestBackendOptionsAreValidatedAndExcludedFromModelInput(t *testing.T) {
	...
	for _, attrs := range []map[string]string{
		...
		{"fast_mode": "yes"}, {"shell_access": "True"}, {"cwd": "/"}, {"approvalPolicy": "never"}, {"model": "bad model"}, {"effort": "bad effort"},
	} {
	...
	if !defaults.FastMode {
		t.Fatal("fast mode should default on")
	}
	explicit, err := ParseBackendOptions(map[string]string{"fast_mode": "false"})
	if err != nil || explicit.FastMode {
		t.Fatal("fast mode off was not preserved")
	}
