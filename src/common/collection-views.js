...
Views.prototype.list=function(kind,state,snapshot,cursor,done,preferCache){
 ...
 this.client.list(kind,state,snapshot,cursor,function(e,p){
  ...
  if(e)return done(e);
  var view=self.begin(kind),titles=[],states=[];
  p.records.forEach(function(r,i){
   view.aliases["r"+i]={record:r,state:state,snapshot:snapshot||"",cursor:cursor||""};
   titles.push(Wire.truncateUtf8(String((kind==="event"?eventTitle(r):r.title)||"").replace(/\s+/g," ").trim()||"Untitled",71));
  ...
  done(null,{token:view.token,list:{kind:kind,state:state,total:p.total,titles:titles,states:states.join(""),next:!!p.next_cursor,stale:!!p.stale,partial:!!p.partial,first:!snapshot&&state==="active",pending:p.pending_count||0}});
 },preferCache);
};
