...
function safe(n){return typeof n==="number"&&isFinite(n)&&Math.floor(n)===n&&Math.abs(n)<=9007199254740991;}
...
function checksum(s){var h=2166136261;for(var i=0;i<s.length;i++){h^=s.charCodeAt(i);h+=(h<<1)+(h<<4)+(h<<7)+(h<<8)+(h<<24);}return (h>>>0).toString(16);}
// Keep exact input comparison without storing the payload a second time.
function inputMetadata(input) {
 var metadata={};
 Object.keys(input).forEach(function(key){metadata[key]=key==="payload"?null:input[key];});
 return metadata;
}
function sameInput(receipt,input) {
 return JSON.stringify(receipt.input)===JSON.stringify(inputMetadata(input)) &&
        JSON.stringify(receipt.operation.payload)===JSON.stringify(input.payload);
}
function pack(data) {
 var packed=Object.assign({},data);
 packed.entries=data.entries.map(function(entry){
  var result={ingress:entry.operation.ingress_id};
...
function unpack(data,schema) {
 if(schema===1){
  Object.keys(data.receipts).forEach(function(key){
   ...
   receipt.input=inputMetadata(input);delete receipt.hash;
  });
 }
 data.entries=data.entries.map(function(entry){
  ...
  var result={operation:receipt.operation};
  if(entry.receipt)result.receipt=entry.receipt;
  return result;
 });
 return data;
...
function valid(raw){
 try{
  ...
  v.data=unpack(d,v.schema);return v;
 }catch(_){return null;}
}
function Journal(storage,options){this.storage=storage;this.options=options||{};this.slot=-1;this.generation=0;var found=false;for(var i=0;i<2;i++){var raw=storage.getItem(PREFIX+i);found=found||!!raw;var v=valid(raw);if(v&&v.generation>this.generation){this.slot=i;this.generation=v.generation;this.data=v.data;}}if(!this.data){if(found)throw new Error("Collection journal is unreadable. Export storage for recovery.");this.data={sequence:0,scope:null,entries:[],receipts:{},bridge:""};}}
Journal.prototype.commit=function(next){var payload=pack(next);if(payload.length*2>(this.options.maxBytes||512*1024))throw new Error("Phone queue is full. Pending input is preserved.");var slot=this.slot===0?1:0, envelope=JSON.stringify({schema:2,generation:this.generation+1,payload:payload,checksum:checksum(payload)});var self=this;
 function write(){self.storage.setItem(PREFIX+slot,envelope);if(self.storage.getItem(PREFIX+slot)!==envelope||!valid(envelope))throw new Error("Phone storage verification failed.");}
 try{write();}catch(e){if(this.options.evictCache){this.options.evictCache();write();}else throw e;}
 this.data=next;this.slot=slot;this.generation++;
};
Journal.prototype.update=function(fn){var next=unpack(JSON.parse(pack(this.data)),2);var result=fn(next);this.commit(next);return result;};
Journal.prototype.enroll=function(scope){this.update(function(d){if(d.scope&&JSON.stringify(d.scope)!==JSON.stringify(scope))throw new Error("Server changed. Export/recover the original queue before enrolling.");d.scope=scope;});};
Journal.prototype.bridge=function(id){this.update(function(d){d.bridge=id;Object.keys(d.receipts).forEach(function(k){if(d.receipts[k].durable)delete d.receipts[k];});d.entries=d.entries.filter(function(e){return !e.receipt||!e.receipt.durably_recorded;});});};
Journal.prototype.accept=function(input){
 var old=this.data.receipts[input.ingress];
 if(old){if(!sameInput(old,input))throw new Error("Event identity reused with different input.");return old.operation;}
 var self=this;return this.update(function(d){if(!d.scope)throw new Error("Configure and connect the collection server first.");
 ...
 if(d.entries.length>=(self.options.maxOperations||256))throw new Error("Phone queue is full. Pending input is preserved.");if(d.sequence>=9007199254740990)throw new Error("Client identity exhausted. Recovery required.");var seq=String(++d.sequence),id="op:"+d.scope.client_id+":"+seq;
 ...
 d.entries.push({operation:op});d.receipts[input.ingress]={input:inputMetadata(input),operation:op,durable:false,alias:input.alias,view:input.view};return op;});};
...
Journal.prototype.compact=function(records){
 ...
 var next=unpack(JSON.parse(pack(Object.assign({},data,{entries:entries}))),2);
