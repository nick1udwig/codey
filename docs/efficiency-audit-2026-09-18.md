...
2. **remove-turn-text-accumulation** — Replaced repeated response concatenation with a text-presence flag. Ordering, early deltas, empty deltas, commentary, multiple items, duplicate completion and stream failure tests pass with the agent and PAM suites. Benchmark allocations for 256/1024/4096 deltas: 22,931/90,640/361,828 bytes, approximately linear.

3. **backoff-watch-delivery-retries** — Added an owned exponential retry timer capped at 60 seconds, disconnect suspension and prompt connection/bridge restart. Pending message identity is retained; duplicate acknowledgments are ignored after confirmation. Simulated one-hour unresponsive and busy periods each produce fewer than 70 callbacks; cancellation, reconnect and duplicate scheduling pass under ASan/UBSan. All 102 phone tests and both SDK builds pass. No physical battery measurement was performed.

4. **shrink-release-pbw-packaging** — Added npm run build:watch:release, SDK identifier minification with preserved license comments, and separate composed source maps with embedded sources and SHA-256 checksums. Release PBW is 233,158 bytes versus 452,913 bytes (48.5% smaller); native payloads and manifests are byte-identical. All 103 tests pass with the generated release bridge, including startup recovery of pending Unicode collection operations. Hardware installation remains release qualification.

5. **index-provider-work-lookups** — Added a nonunique binding/record mapping index and two partial unfinished-job indexes preserving rowid order. Existing v2 stores acquire them atomically on reopen. Store/provider suites pass; plans verified at 100/10,000/100,000 historical jobs across seven unfinished states. At 100,000 history: 100 reads 2.57 ms, migration 25.34 ms, database +12 KiB; 1,000 pending inserts in a rolled-back transaction increased from 7.62 to 11.89 ms. Timings are local fixtures, not production results.

6. **batch-provider-record-imports** — Added transaction-scoped imports with reused statements, 32-record chunks and a 25 ms elapsed-work yield threshold (one record/commit may exceed it). Engine checkpoints follow successful durable batches. Fault rollback/replay and checkpoint tests plus store/provider/engine suites pass. Local 1,000-record changed fixture: transactions 1000→32, elapsed 276.6→182.9 ms, WAL 60.15→15.32 MB, maximum concurrent read 0.84→7.13 ms. Mixed elapsed 165.3→111.1 ms; unchanged 66.7→27.0 ms with zero WAL in both cases.

7. **budget-phone-collection-cache** — Introduced a disposable cache module with 256 KiB/48-page/128-record budgets, recency-based selective eviction, current/first-page preference, revision-pinned page data, cached serialized entries and unchanged-write suppression. A 100-page small-budget test retained current navigation and collection metadata, evicted 95 pages, serialized 134,560 bytes and wrote 714,740 bytes; persisted data stayed under 16,000 bytes. Offline reopen, scope isolation, failed writes and existing overlay/journal tests pass, along with 103 phone tests and both SDK builds. Phone heap and physical energy were not measured.

8. **normalize-phone-journal-payloads** — Journal schema 2 persists one operation payload and entry references, while exact input metadata/payload comparison remains available for replay. Updates preserve shared operation objects and duplicate accepts skip writes. Legacy slots load without writes and migrate through verified alternating commits. Torn migration, readback failure, restart, mismatches, dependencies, quota and bridge rollover tests pass. Estimated UTF-16 payload sizes: 20 notes 163,410→50,284 bytes; 100 notes 816,704→250,772. Cumulative envelope writes were 564,642 and 13,437,034 bytes respectively. Live replay receipts keep their bridge-scoped lifecycle and byte ceiling. Phone tests and generated release bundle tests pass; physical phone transient heap was not measured.

9. **bound-note-body-page-cost** — Schema 3 stores revision bodies in indexed 4 KiB chunks with separate length/hash/completeness metadata, removes body duplication from revision JSON, and reconstructs full revisions for edits/providers. Migration is atomic; backups include chunks. Unicode boundary, old revision, empty body, cursor isolation, event-description reconstruction, migration and backup tests pass, as does the full Go suite. First/middle/final 704-byte page benchmarks at 64/256 KiB stay around 31–36 µs and 17–20 KB allocated per read. Large existing stores need startup time and WAL space for backfill; a single record can still exceed the import elapsed-work target.

10. **reduce-native-element-copy-work** — Screen begin now resets the live count instead of clearing 39,168 element bytes; add still initializes each live slot. Field application reports normalized stored-value changes and skips no-op refreshes. Extracted element logic has sanitizer tests for partial/empty/flag patches, truncation and UTF-8 boundaries. Audited live-count readers and guarded/reset stale touch-control indices. Both SDK builds pass. Disassembly shows agent_ui_patch own stack frame falling from 840 to 16 bytes on both targets (excluding callees); the permanent element array is unchanged. End-to-end visual/gesture qualification remains a hardware check.

11. **separate-watch-clock-invalidation** — Added independent geometry/content/action/clock invalidation. Static note/form minute ticks request only the clock layer; dashboard/calendar request content without geometry. Status text keeps geometry invalidation when its presence changes. Hidden windows defer pending paints until appearance, while structural changes can close an old NumberWindow. Native sanitizer policy tests cover ten clock ticks, data invalidation, passive throttling, input and hidden/structural cases; both SDK builds pass. SDK layer compositing and both physical display shapes still need release qualification.

12. **extract-bridge-renderer-boundaries** — Extracted collection orchestration and job presentation/action acknowledgments behind injected request/transport/settings interfaces. Expanded dense journal/client routines without changing serialized format. Native dashboard drawing/layout uses a narrow read-only view plus artwork cache; gesture recognition has a separate controller, and AgentUi keeps widget ownership. Entry point is 521 lines and AgentUi 1,999 lines. All 103 source and generated-release phone tests, collection recovery tests, native sanitizers and both SDK builds pass. The release PBW is now 239,118 bytes; refactoring is for maintainability, not a claimed binary-size gain.

13. **reclaim-agent-session-bookkeeping** — Added an owner-and-waiter-counted session lock registry and generation-scoped loaded-thread tracking that ignores late replies. Session mappings now use incremental SQLite writes and indexed reads, atomically importing legacy JSON once without deleting old conversations. Read/write failures propagate. Race tests cover 2,000 contending acquisitions, 5,000 completed sessions, 100 canceled waiters and 100 connection generations; registries reclaim inactive state. Migration/restart/deletion/old-job/failure tests pass. Writes after 100 versus 10,000 mappings measured 13.03 versus 13.11 µs, about 215 bytes allocated. The full npm suite passes, including 13 integration tests and all Go packages. Session backup/downgrade requirements are documented in docs/server.md.

**Task 3 follow-up.** Final review follow-up: distinguish phone handoff from terminal delivery acknowledgment. Permit accepted_phone → accepted_server/needs_attention while suppressing duplicates and delayed regressions. Native sanitizer transition tests and both SDK builds pass.

**Task 7 follow-up.** Final review follow-up: cache focus is assigned when navigation starts, including offline reads. Late responses may populate cache entries without stealing focus. Out-of-order response and offline back-navigation eviction tests pass, together with collection and source/generated-release phone tests.
